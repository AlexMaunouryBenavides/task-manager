...
describe("POST /api/register", () => {
  it("should return token for valid login", async () => {
    ...
    const user = [
      { id: 1, email: "truc@test.fr", password: hashedPassword },
    ] as Rows;
    ...
    const res = await supertest(app)
      .post("/api/v1/login")
      .send({ email: "truc@test.fr", password: "hach" });
  ...
  it("should fail on invalid user", async () => {
    ...
    // resultat attendu
    // devrait etre 400 au lieu de 500 mais je n'ai pas
    // de validation correct
    // donc je passe dans le next
